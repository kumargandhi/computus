...
export class MortgageResultsComponent implements OnInit {
    ...
    calculate() {
        ...
        do {
            month = month + 1;
            const monthlyPayment: MonthlyPayment = getBalance(
                loanAmount,
                this.computedValues.interestRate,
    ...
    private _getPaymentInformation(): any {
        ...
        for (let i = 0; i < this.monthlyPayments.length; i++) {
            tableData.push({
                Month: this.nf.transform(
                    this.monthlyPayments[i].monthlyResult.month
                ),
                Interest: this.nf.transform(
                    this.monthlyPayments[i].monthlyResult.interest
                ),
                Principal: this.nf.transform(
                    this.monthlyPayments[i].monthlyResult.principal
                ),
                Balance: this.nf.transform(
                    this.monthlyPayments[i].monthlyResult.balance
                ),
            });
        }
        return tableData;
